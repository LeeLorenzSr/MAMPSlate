...
- `/api/v1/...`: Versioned CRUD API (articles, pages, media, comments). See [api-v1.md](api-v1.md).
...
- **Versioned API**: `/api/v1/...` provides capability-gated CRUD for articles, pages, media, and comments, authenticated by API key or temporal session key (no CSRF). See [api-v1.md](api-v1.md).
...
The `features` config block (`articles`, `comments`, `media`, `categories`,
`tags`, `seo_sitemap`, `rss_feed`) lets a copied project disable unused
subsystems. Disabled features hide their admin nav and return 404 on their
routes via the `feature()` / `requireFeature()` helpers in `includes/http.php`.
