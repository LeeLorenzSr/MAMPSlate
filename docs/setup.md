...
3. **Initialize schema** — runs the migration runner, which applies pending
   `sql_init/001`–`016` migrations in order (idempotent; safe to re-run) and
...
- The database schema (`sql_init/001`–`010`) and the initial administrator row.
...
least-privilege user, apply `sql_init/001`–`010`, and copy
